...
parser.add_argument('--jobs', type=int, choices=[1, 2], default=1,
                    help='Per-tool parallelism; one by default for Android process pressure')
args = parser.parse_args()
lock = (base/'.build.lock').open('a')
try:
    fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
except BlockingIOError:
    print('BUILD_NOT_STARTED: phone build slot busy; retry after the active build completes', flush=True)
    raise SystemExit(75)
...
if not all(p.exists() for p in [build/'android-build.json', build/'extra-inputs/toolchains.json']):
    print('BUILD_NOT_STARTED: verified toolchain preparation must complete first', flush=True)
    raise SystemExit(75)
...
env.update({'ORG_GRADLE_PROJECT_android.aapt2FromMavenOverride': config['aapt2'],
            'PERL': '/usr/bin/perl',
            'GRADLE_OPTS': f'-Dorg.gradle.daemon=false -Dorg.gradle.workers.max={args.jobs}',
            'GOMAXPROCS': str(args.jobs), 'GOFLAGS': f'-p={args.jobs}',
            'CARGO_BUILD_JOBS': str(args.jobs), 'CMAKE_BUILD_PARALLEL_LEVEL': str(args.jobs),
            'TINYAGENT_BUILD_JOBS': str(args.jobs), 'ANDROID_NDK_HOME': extra['ndk_home']})
task = ':app:assembleFreeDebug' if args.workload == 'antennapod' else ':app:assembleDebug'
command = ['/usr/bin/bash', './gradlew', task, '--no-daemon', f'--max-workers={args.jobs}',
...
if args.workload == 'organic-maps':
    ...
    command += ['-Parm64', f'-Pnjobs={args.jobs}']
    patterns = ['android/app/build/outputs/apk/**/*.apk']
if args.workload == 'vlc-android':
    # VLC's Meson sub-build does not inherit Make's job limit. Pass its supported make variable too.
    env.update(ANDROID_NDK=extra['ndk_home'], ANDROID_SDK=config['android_home'],
               MAKEFLAGS=f'-j{args.jobs} MESONCOMPILEFLAGS=-j{args.jobs}')
...
with lock:
    ...
    report = dict(workload=workload['name'], commit=revision, cwd=str(cwd), command=command,
                  java_home=env['JAVA_HOME'], android_home=env['ANDROID_HOME'], status='running', jobs=args.jobs,
