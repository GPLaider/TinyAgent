...
assert 'FAILED (failures=2)' in files['before.log']['content']
assert 'exit_code=1' in files['before.log']['content']
assert 'Ran 3 tests' in files['after.log']['content']
assert '\nOK\n' in files['after.log']['content']
assert 'exit_code=0' in files['after.log']['content']
...
assert any('before_test_exit_code=1' in p.get('output', '') for p in bash)
assert any('after_test_exit_code=0' in p.get('output', '') for p in bash)
